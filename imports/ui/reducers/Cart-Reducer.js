...
const CartReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case 'REMOVE_CART_ITEM_SUCCESS' : {
            let newUserCartItems = state.items.filter( item => {return item.productId !== action.productId});
            let newTotalAmount = state.totalAmount - (parseFloat(action.price) *  parseFloat(action.quantity));
