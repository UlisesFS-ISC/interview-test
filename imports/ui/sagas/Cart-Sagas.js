...
function loadCart(action) {
    return eventChannel( emitter => {
        Meteor.call("carts.getCartByUserName",
            action.userName,
            (error, response) => {
                if (error){
                    emitter(error);
...
function removeItem(action) {
    return eventChannel( emitter => {
        Meteor.call("merchants.rollBackProductAvailability",
            action.merchantId, action.productId, action.quantity,
            (error, response) => {
            if (error) {
                emitter(error);
                emitter(END);
        ...
        Meteor.call("carts.removeItemFromUserCart",
            action.userName, action.productId,
            (error, response) => {
                if (error) {
                    emitter(error);
                    emitter(END);
...
function submitOrder(action) {
    return eventChannel( emitter => {
        ...
        Meteor.call("orders.insertOrder",
            action.order,
            (error, response) => {
                if (error) {
                    emitter(error);
        ...
        Meteor.call("carts.emptyUserCart",
...
export function* removeCartItemCalls(action) {
    ...
    try {
        while (true) {
            ...
            yield put(CartActions.removeCartItemSuccess(action.productId, action.price, action.quantity));
        }
    } catch (e) {
        yield put(CartActions.showMessage(e.message, true));
    }
